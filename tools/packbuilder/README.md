...
- `passage_retag(toks)`: rewrites the tagged `[text, lemma, upos, morph]` tokens (list copies) before linking. ru: "Тому, кто" is the correlative тот, not Tom; стоит/стоят is стоить with a price, "того", "ли" or an infinitive, else стоять; mid-sentence "Новый год" is новый + год; the sign-off "Целую," is целовать; меньше is мало.
