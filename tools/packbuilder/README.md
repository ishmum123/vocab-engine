...
- `passage_ruby(lk, passages, names_of)` (spec-level, see the hook below; only when `pack/characters.json` exists): `sentences[].ruby`, `titleRuby`, `questions[].ruby` and `questions[].optionsRuby` (docs/PACK_SCHEMA.md passages.json), so kana-first display never shows a kanji below mastered or composes a reading per character (6時 is 6 + 時 じ, 一週間 いっ + しゅうかん, 一杯 いっぱい). Every kanji is inside a token; nothing is skipped. `text_ruby`:
  ...
  The report's "Readings" section gives token counts, null wordIds, fallbacks, cut segments, word-reading overrides, the kana rules applied, and what the `sentences.json` rules (`sentence_ruby`) would have skipped on the same sentences.
