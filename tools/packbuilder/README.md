...
`Linker.pretag` sets `spec.passage_tagging` (default False, restored afterwards, also on error) for the whole passage tagging run (`passage_text`, `tag_texts`, `fix_token`/`fix_sentence`, `passage_retag`). Spec code that writes a derived cache under `.cache/derived` from inside tagging checks `core.util.derived_write_ok(spec)` and skips the write: ja `_build_groups` (the word-groups file is keyed by the corpus but built from the texts handed to the tagger, so passage tagging would overwrite it), fa's Stanza memo (passage texts stay in memory). The defect class is any cache whose contents depend on the tagger input but whose key does not. Such an aggregate (ja word groups) is written only while `core/tag.py` `stage_tag` tags the cached corpus (`spec.corpus_tagging`, checked by `core.util.corpus_write_ok`), never by another `tag_texts` caller: passages, `example_rows` via `tag_rows`, a script or test tagging a few texts. On 2026-09-26 a one-sentence debug `tag_texts` call rewrote `ja_groups_t22_*.json.gz`; the next passages link context moved 241 word ids (cold and warm builds, and two cold builds with different `PYTHONHASHSEED`, are otherwise byte-identical). The groups file stores its text count, and a build that reads it fails when the count differs from the tagged corpus's `sentences` (`tagged_*.meta.json`); a file written before the count existed is logged as unverifiable. Per-text memos (fa/hi/ur Stanza, ar camel) map a text to its analysis, so extra texts only extend them; they keep `derived_write_ok`. As a tripwire, `pretag` compares every file in `.cache/derived` (size, mtime) before and after tagging and fails the run naming any that changed. Caches built in `load_context` (the build's own passes over the corpus and dictionaries, and the passages context pickle) are not affected. ko (in-memory analyser) and zh (`passage_linker`, no tagger) write nothing while tagging. The context pickle stores `spec.__getstate__()` (the instance `__dict__` unless a spec overrides it; ja drops its unpicklable Sudachi tokenizer).
...
- `tag_texts` while `passage_tagging`: the word groups (display lemma per Sudachi atom) are built once over the cached corpus plus the passage texts, as in the build, not from the passages alone (いつ is not 何時 なんじ); `_build_groups` does not save them. Any other caller outside the corpus tag stage (example rows, scripts) gets groups over the cached corpus alone, built once in memory and never saved.
- `__getstate__`: the Sudachi tokenizer is dropped for pickling and rebuilt on use.
- `passage_ruby(lk, passages, names_of)` (spec-level, see the hook below; only when `pack/characters.json` exists): `sentences[].ruby`, `titleRuby`, `questions[].ruby` and `questions[].optionsRuby` (docs/PACK_SCHEMA.md passages.json), so kana-first display never shows a kanji below mastered or composes a reading per character (6時 is 6 + 時 じ, 一週間 いっ + しゅうかん, 一杯 いっぱい). Every kanji is inside a token; nothing is skipped. `text_ruby`:
  - kana segments: `_sudachi_pieces` (Sudachi per morpheme, `READING_FIX`) through the same context rules as `sentences.json` (`_kana_segments`: 一人/二人, 何 なに/なん, day counts, minutes, 中 じゅう/ちゅう, digits kept) plus two passage rules: `_counter_sounds` (a number + counter in `COUNTER_SOUNDS` takes its sound change: 一杯 いっぱい, 三階 さんがい, 何本 なんぼん, 1杯 1ぱい, a kanji numeral + つ native: 四つ よっつ) and `_passage_kana_rules` (来 by the kana after it: 来る く, 来ない こ, 来ます き; 言 い, not Sudachi's colloquial ゆ);
  - tap segments: a sentence's `spans` and the gaps between them (titles, questions, options: the linker's spans for that text). A token never crosses a segment edge (the engine renders such a token plain): a kana segment crossing one is cut there (`_split_reading`: a kanji-free side reads as written; a digit + 日/人 keeps the counter's tail, 3日 3|か; else one side's Sudachi or pack reading that begins or ends it, rendaku allowed; else each side's own Sudachi reading, the listed fallback);
  - a span holding a kanji is one token over its segments, the edge kana it shares with the text left out (`_token_ruby`, as `sentence_ruby`: 行く 行 い, 悪かっ 悪 わる); when that token is exactly the kanji run of its linked word's headword, the word's own reading wins (`_agree_word`: 箱 はこ, その間 あいだ, 毎日|外 そと; not for 来, 何, 数 or numerals); after a kanji or digit only when that character ends another span, so a suffix after a name keeps Sudachi's reading (松本城 じょう); wordId is the span's word when it is a characters.json unit's `words[0]` (and in the sentence's `words`), else null;
...
- The linker may also define `passage_ruby(passages, names_of)`: called on the built passages before writing (also with `--check`), it adds reading fields in place and returns extra report lines (appended after the notes). zh: per-token readings, see "Chinese (zh) passages". Without one, a spec's `passage_ruby(lk, passages, names_of)` is called the same way (ja: see "Japanese (ja) passages").
