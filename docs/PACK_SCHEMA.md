...
| `typing.accents` | `"lenient"` or `"strict"` | no (`"lenient"`) | With lenient, optional marks are folded on both sides: Latin accents (`perche` = `perché`), stress marks (`молоко́`), Arabic-script harakat and tatweel, Hebrew points, and ZWNJ/ZWJ (`میروم` = `می‌روم`). Lenient also folds optional spelling variants in Arabic script and Devanagari (hamza carriers, ء, ة, ى, ھ, nukta, chandrabindu; see "Lenient typing letter folds" below), and, for the German pack only (`key === "de"`), German ASCII substitutions: ä → ae, ö → oe, ü → ue, ß → ss, folded on both sides before the Latin accent strip so `Muede`/`Strasse` type as `müde`/`Straße` and typed ä/ö/ü/ß still match (core.js `GERMAN_ASCII`, `foldGermanAscii`; see "German ASCII substitutions" below). Every lenient fold is guarded: an answer that matches only after folding is wrong when it is exactly another pack word (`si` for `sí`). Marks that make a different letter are kept: Cyrillic й and ї, Devanagari vowel signs, and kana voicing marks. Strict never folds them. Arabic kaf and yeh always equal their Persian/Urdu forms (ک, ی), in both modes, because keyboards produce either. |
...
With `typing.accents: "lenient"` and `key === "de"` (at levels before `strictFromLevel`), typed answers also fold ä → ae, ö → oe, ü → ue and ß → ss on both sides (core.js `GERMAN_ASCII`, `foldGermanAscii`), gated to the German pack so no other pack's text is affected. This runs before the Latin accent strip (`foldAccents`) so ä/ö/ü keep their digraph rather than being stripped to a bare vowel: `Muede` and `Straße` both type `müde`/`Straße` correctly, and typing ä/ö/ü/ß directly still works. This is a different, more specific fold than the plain Latin accent strip every other Latin-script pack gets (which would otherwise turn ö into a bare `o`): it changes what, if anything, collides for German specifically, since ä/ö/ü/ß are consumed before the plain strip ever sees them. For example `schon`/`schön` and `zahlen`/`zählen` (real German pack words, w0054/w0109 and w0671/w0706) no longer fold to the same key at all under this substitution (`schön` → `schoen`, not `schon`; `zählen` → `zaehlen`, not `zahlen`), so typing one for the other is simply wrong, not merely guarded. An audit of `german/pack/words.json`'s `w` and `alt` fields found no pair that newly collides under this fold; the collision guard below still covers the class generically if one is added later (proven with a synthetic `Masse`/`Maße` pair in `tests/engine_checks.js`, since ß → ss can still collide two real words the way any other lenient fold can).
