...
// Tradeoff: distinct words can become one typed answer (ماء "water" = ما "what"). The
// per-pack collision lists are in docs/PACK_SCHEMA.md (typing section).
...
// words (optional, the pack's WORDS): collision guard for LENIENT_LETTERS. An answer that
// matches only because of those letter folds (not already strictly or by plain accent
// folding) is rejected when it is exactly another pack word's w/alt (strict form): typing
// ماء "water" for ما "what" is wrong, typing امس for أمس is fine. Plain accent folds
// (Spanish si/sí) are not guarded.
function acceptTyped(input, entry, pack, extra, words){
  ...
  const base = { caseSensitive: cs, foldAccents: lenient, lenientLetters: false };
  const got = normalizeTyped(input, base);
  if(!got) return false;
  if(targets.some(x => normalizeTyped(x, base) === got)) return true;
  ...
  const hits = strictFormIndex(words, cs).get(normalizeTyped(input, { caseSensitive: cs })) || [];
