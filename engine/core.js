...
function buildReviewPlan(learned, prog, pack, opts){
  ...
  if(ru.length) return unifiedReviewPlan(learned, ru, prog, pack, n, o.rng);
...
function stagePath(pack, words, units, prog){
...
function nextStage(pack, words, units, prog){ return stagePath(pack, words, units, prog).find(s => !s.done) || null; }
...
function charsStarted(pack, words, units, prog){
  if(!charsConfig(pack)) return false;
  if(Object.keys(charRecs(prog)).length) return true;
  const st = nextStage(pack, words, units, prog);
...
// One ranking over words and units: the n weakest as [{kind:"w"|"c", entry, score}],
// with weakFirst's jitter (below 1, so it only reorders equal scores).
function rankUnified(words, wrecs, units, crecs, n, pack, rng){
  const r = rng || Math.random;
...
// a random pack reviewKind. Items: {kind, word} or {kind, unit}.
function unifiedReviewPlan(learned, ru, prog, pack, n, rng){
  const cfg = charsConfig(pack); const r = rng || Math.random;
  const pv = provPick(learned, Math.min(REVIEW_PROV, n), prog.w);
  const pvSet = new Set(pv.map(w => w.id));
  const ranked = rankUnified(learned.filter(x => !pvSet.has(x.id)), prog.w, ru, charRecs(prog), n - pv.length, pack, rng);
  ...
  let wi = 0;
  return pool.map(x => x.kind === "w" ? { kind: kinds[wi++], word: x.entry }
    : { kind: cfg.reviewKinds[Math.floor(r() * cfg.reviewKinds.length)], unit: x.entry });
}
...
function todaySnapshot(pack, words, units, prog){
  const stage = nextStage(pack, words, units, prog);
  const cset = stage && stage.kind === "chars" ? nextCharSet(stage.levels, units, pack, prog) : null;
  const started = charsStarted(pack, words, units, prog);
  return { stage, cset, charsStarted: started, reviewSize: started ? REVIEW_SIZE_CHARS : REVIEW_SIZE, choice: showCharChoice(pack, words, units, prog) };
}
