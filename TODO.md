...
- **zh types pinyin, not hanzi** (brief BP2, `typing: "pron"`). Typing hanzi needs an IME, so the production slot that alternates with recall types the word's pinyin (tone marks or numbers, hsk's rules; toneless is wrong with a "tones missing" note). Pinyin is never drilled on its own and there is no typed cloze. Residual: a word whose pron is written with tone sandhi (一点儿 yìdiǎnr) accepts only that form, not the citation tones.
