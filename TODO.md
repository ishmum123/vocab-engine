...
- **RTL gloss popover** (Persian live check 2026-09-25): `#gloss` stays LTR for RTL packs, so the tapped word sits flush left with the English after it; only the inner `.gw` span is dir=rtl (PACK_SCHEMA.md ~143). For `pack.rtl` set `dir=rtl` + `text-align:start` on the popover container so the word anchors at the right edge and the pron/gloss follow in reading order; keep the English gloss span LTR. Bundle with the next engine republish.
- **Load-time follow-ups (measured on the German site 2026-09-25):** index.html is a single 1.36 MB file (380 KB gzipped) because the pack is inlined; sentences + passages are 979 KB raw / 281 KB gz of it, words 242 KB, engine ~140 KB. GitHub Pages serves gzip with `cache-control: max-age=600`, so every visit after 10 minutes revalidates (304 unless republished). In order of value: (1) ~~service worker with a build-hash cache name~~ done 2026-09-25 (build.sh writes sw.js; README "Offline and repeat loads"). Language repos must now publish sw.js with index.html, and their check.sh stale guard compares index.html only, so a stale sw.js is not caught there. (2) Load passages as a separate chunk on first Read-tab open (~100 KB gz saved for learners who never read). (3) Drop the render-blocking Google Fonts stylesheet: self-host the two weights actually used or use the system stack; `preconnect` + `display=swap` already limit the cost. (4) Ship the pack as JSON strings parsed with JSON.parse instead of object literals (faster parse of large literals on phones; small). Runtime hot paths were not profiled; Today's scans over 3.4k sentences are cheap. Do after the current republish wave so all packs pick it up in one republish.
