...
- Read tab: inflected forms in passage text (mele, compra, va, vecchia) are not tappable inline; they appear as lemma chips under the sentence (Italian pilot: 4020 inline links vs 1955 chips). Fix: `packbuilder passages` already tags the text, so emit per-sentence `spans` [[start,end,wordId]] in passages.json; engine prefers spans when present and falls back to surface matching. Same idea would help highlight inflected headwords in ordinary example sentences.
