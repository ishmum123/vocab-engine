...
- **No-voice notice timing** (German live check 2026-09-25): `speechNotice()` fires on whichever listening item is built first, and the items are then shuffled, so the one-time notice can appear mid-session (item 11 of 20) instead of on the first shown item. Fire it when the first listening item is *shown*, not built.
